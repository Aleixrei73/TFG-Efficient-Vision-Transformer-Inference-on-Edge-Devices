...
def trainKD_step(teacher: torch.nn.Module,
    ...
    for batch, (X, y) in enumerate(dataloader):
        ...
        # Calculate and accumulate accuracy metric across all batches
        y_pred_class = torch.argmax(torch.softmax(student_pred, dim=1), dim=1)
        train_acc += (y_pred_class == y).sum().item()/len(student_pred)
...
def trainKD(teacher: torch.nn.Module, 
    ...
    for epoch in tqdm(range(epochs)):
        ...
        train_loss, train_acc = trainKD_step(teacher=teacher,
                                           student=student,
                                           dataloader=train_dataloader,
                                           loss_fn=loss_fn,
                                           temp=temp,
        ...
        if test_acc > current_max:
            ...
            # Save the model state_dict()
            print(f"[INFO] Saving model to: {model_save_path}")
            torch.save(obj=student.state_dict(),f=model_save_path)
